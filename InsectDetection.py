...
<style>
    ...
    /* Fixed text color for unselected tabs */
    .stTabs [data-baseweb="tab"] div {{
        color: {TAB_TEXT} !important;
    }}
    /* Fixed text color for selected tabs */
    ...
    /* Ensure Info messages are readable */
    .stAlert {{ background-color: {SURFACE} !important; border: 1px solid {BORDER} !important; }}
    ...
    with ref_cols[i % 3]:
        st.markdown(f"""
            <div style="padding:15px; border-radius:15px; border:1px solid {BORDER}; background:{SURFACE}; margin-bottom:10px;">
                <b style="color:{TEXT}; font-size:1.1rem;">{name}</b><br>
                <span style="color:{data['color']}; font-size:0.75rem; font-weight:800;">{data['status'].upper()}</span><br>
                <span style="color:{TEXT_DIM}; font-size:0.85rem; line-height:1.4;">{data['desc']}</span>
            </div>
        """, unsafe_allow_html=True)
